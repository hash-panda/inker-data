<template>
  <a-card class="general-card" :bordered="false">
    <template #title>
      {{ $t('nftProsperPeacockAirdrop.title') }} ({{
        nftAirdropInfo?.length ?? 0
      }})
    </template>
    <template #extra>
...
import { nftProsperPeacockAirdrop } from '@/utils/check-prosper-peacock-nft';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const fetchData = async (address: string, index: number) => {
      try {
        const isAirdropNft =
          nftProsperPeacockAirdrop.findIndex((v) => v === address) >= 0;
    ...
    const checkEligible = () => {
      window.open(
        'https://docs.google.com/spreadsheets/d/1bPn_cqruF13oei1PvfYkEKpimRhlplpG55mO1N1lxvA/edit#gid=0',
        '_blank'
      );
    };

    const openWeb = (address: string) => {
      window.open(`https://knowhere.art/profile/${address}`, '_blank');
    };
